...
| *(ce commit)* | **Écrans de config en maître-détail + fin des redondances** (demande du 01/08, lot 1/…) : un composant `SideRail` réutilisable met le sous-menu à gauche et le détail à droite. **Général** perd ses éditeurs de listes (partenaires, modalités, sous-types de PI, activity tags, catégories) — dupliqués avec « Listes paramétrables » — et ne garde que l'identité et le barème. **Indicateurs** passe du triple filtre à un **rail par niveau** (CRF Outcome/Output/Other output/Transversal + XLSForm), catégorie et recherche à droite. **Calculs** passe des cartes empilées à un **rail** (base / personnalisés / Variables utilisables). L'entrée « activités » quitte le rail des listes typées (masquée, `hidden`) — l'onglet **Activités** en est le seul foyer, et reçoit le bouton de **renommage de tag en cascade** (super). Tests : 263 serveur, 45 web, build propre. | Répond aux points 1, 6, 7, 8 de la demande. **Constat** : l'évaluation des formules (`new Function`) est bloquée par la CSP dans le build servi — à corriger par un évaluateur sûr (hors lot UI). **Restent** : hiérarchie des bureaux (2), localités/import sites (3), périmètre dans le bureau (4), sous-activités (5), variables XLSForm (9, en attente des fichiers). |
| `93b5c9f` | **Paramètres catégorisés + fiche pays complète** (demande du 01/08) : la configuration passe d'une rangée d'onglets qui débordait à un **rail de catégories** — Configuration, Référentiels, Organisation & lieux, Collecte & sources, Calculs & rations, Restitution & accès — chacune ouvrant ses sous-écrans, avec fil d'Ariane « catégorie › sous-catégorie ». Et le découpage quitte les cartes séparées pour rejoindre **la fiche du pays** (« Pays & découpage ») : on ouvre le pays, on **injecte le shapefile et le dbf** dans son interface, on gère les contours par niveau, et **en bas un récapitulatif par niveau** (unités / contours / complétude, total). Tests : 263 serveur, 45 web (l'e2e ouvre la fiche du pays et vérifie l'injection, les contours et le récap). | Répond à « l'UI n'est pas catégorisée avec des sous-catégories » et « sur l'interface d'insertion des données du pays, injecter le shapefile et les dbf, et en bas un récap par niveau ». |
| `d7809da` | **L'activité EST son tag, en valeur unique** (migration 028) : « liste des activités = activity tag (valeur unique) ». Le schéma imposait l'unicité du NOM — un libellé, qui se traduit et se reformule — et laissait le TAG libre, alors que **dix colonnes le portent en texte** comme clé de jointure : deux activités partageant un tag rendaient ces colonnes ambiguës. La migration **normalise** (espaces intérieurs retirés, majuscules : « FBA _CCS » = « FBA_CCS »), **fusionne** les doublons en reportant les clés étrangères sur la ligne la plus ancienne — aucun site détaché — puis **contraint** par un index unique insensible à la casse. La route refuse le doublon en NOMMANT l'activité en place, le semis réel se rapproche par tag seul et désambiguïse un nom déjà pris plutôt que de perdre une ligne. Tests : 263 serveur, 45 web. | Répond à la demande du 01/08. Vérifié aussi hors suite : trois activités « URT », « urt » et « UR T » portant chacune des sites fusionnent en une, tous les rattachements préservés. |
