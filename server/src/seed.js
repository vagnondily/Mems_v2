...
/* Référentiel standard de sécurité alimentaire et de nutrition (CARI/PAM,
   IPC/CH, FANTA, ANJE-OMS). Miroir de D_INDICATORS côté client : mêmes codes,
   mêmes libellés — codes en identifiants valides pour les formules composées. */
const INDICATORS = [
  ...
  ["FCGP","Ménages en consommation alimentaire pauvre (FCS)","Consommation alimentaire","%",10,"down","Enquête ménage","Semestriel"],
  ["HDDS","Score de diversité alimentaire des ménages","Consommation alimentaire","idx",6,"up","Enquête ménage","Annuel"],
  ["rCSI","Indice réduit des stratégies d'adaptation alimentaire","Stratégies d'adaptation","idx",12,"down","Enquête ménage","Semestriel"],
  ["CSI","Indice des stratégies d'adaptation (contextuel)","Stratégies d'adaptation","idx",20,"down","Enquête ménage","Semestriel"],
  ["LCSI","Stratégies d'adaptation des moyens d'existence — aucune stratégie","Stratégies d'adaptation","%",60,"up","Enquête ménage","Annuel"],
  ["LCSIE","Ménages recourant à des stratégies de crise ou d'urgence (LCSI)","Stratégies d'adaptation","%",20,"down","Enquête ménage","Annuel"],
  ["FES","Part des dépenses consacrées à l'alimentation","Économie du ménage","%",65,"down","Enquête ménage","Annuel"],
  ["ECMEN","Capacité économique à couvrir les besoins essentiels","Économie du ménage","%",60,"up","Enquête ménage","Annuel"],
  ["HHS","Échelle de la faim des ménages — faim modérée à sévère","Faim et vécu","%",10,"down","Enquête ménage","Semestriel"],
  ["FIES","Insécurité alimentaire vécue — modérée ou sévère (FIES)","Faim et vécu","%",30,"down","Enquête ménage","Annuel"],
  ["CARI","Ménages en sécurité alimentaire (synthèse CARI)","Sécurité alimentaire (synthèse)","%",60,"up","Enquête ménage","Annuel"],
  ["MAD","Régime alimentaire minimum acceptable — enfants 6 à 23 mois","Nutrition","%",35,"up","Enquête ménage","Annuel"],
  ["MDD","Diversité alimentaire minimale — enfants 6 à 23 mois","Nutrition","%",50,"up","Enquête ménage","Annuel"],
  ["MMF","Fréquence minimale des repas — enfants 6 à 23 mois","Nutrition","%",55,"up","Enquête ménage","Annuel"],
  ["MDDW","Diversité alimentaire minimale des femmes (15 à 49 ans)","Nutrition","%",50,"up","Enquête ménage","Annuel"],
  ["GAM","Prévalence de la malnutrition aiguë globale","Nutrition","%",5,"down","Enquête nutritionnelle (SMART)","Annuel"],
  ["SAM","Prévalence de la malnutrition aiguë sévère","Nutrition","%",1,"down","Enquête nutritionnelle (SMART)","Annuel"],
  ["STUNT","Prévalence du retard de croissance (malnutrition chronique)","Nutrition","%",25,"down","Enquête nutritionnelle (SMART)","Annuel"],
  ...
  ["SAMR","Taux de guérison du traitement de la malnutrition aiguë sévère","Nutrition","%",75,"up","Registres des sites","Mensuel"],
  ["PLW","Femmes enceintes et allaitantes recevant un supplément nutritionnel","Nutrition","%",80,"up","Registres des sites","Trimestriel"],
  ...
  ["ABI","Indice de bénéfice des avoirs communautaires créés ou réhabilités","Résilience et avoirs","idx",3,"up","Suivi des actifs","Annuel"],
];
const GEO = [
...
async function seed(){
  const existing = (await db.prepare("SELECT COUNT(*) c FROM sites").get()).c;
...
if(info){
  ...
  await tx(async (db) => {
    ...
    for(const [name,code] of OFFICES){
      const id = newId("off"); const hq = code === "HQ";
      officeId[name] = id;
      await insOffice.run(id, name, code, hq ? "hq" : "field", hq ? "national" : "geo",
    ...
    for(const [name,tag,area] of CATS){
      const id = newId("cat");
      catId[tag] = { id, name, area };
      await insCat.run(id, name, tag, area);
    }

    const partnerId = {};
    const insP = db.prepare("INSERT INTO partners (id,name) VALUES (?,?)");
    for(const n of PARTNERS){
      const id = newId("part"); partnerId[n] = id;
      await insP.run(id, n);
    }

    const insPoi = db.prepare("INSERT INTO poi_subtypes (id,label,code) VALUES (?,?,?)");
    for(const [l,c] of POI) await insPoi.run(newId("poi"), l, c);
    ...
    for(let gi = 0; gi < GEO.length; gi++){
      const g = GEO[gi];
      await insGeo.run(newId("geo"), g[0], g[1], g[2], g[3], CENTRE[gi][0], CENTRE[gi][1]);
    }
    ...
    /* TODO-PG: writeVersion() (lib/geo.js) parle à la base via l'import direct de
       ../db.js (le pool), pas via le client dédié de cette transaction — ses
       écritures s'exécutent donc hors de la transaction FORCE_SEED (autocommit sur
       une autre connexion), et non plus atomiquement avec le reste du jeu de
       données comme sous better-sqlite3 (connexion unique). Signalé, non corrigé :
       la correction suppose de faire accepter un exécuteur `db` à lib/geo.js, hors
    ...
    for(const [office, cats] of PLAN){
      for(const [tag, count, dur, risk, feas] of cats){
        ...
        for(let k=0;k<count;k++){
          ...
          /* La table est écrite en paramètres nommés (@col) sous better-sqlite3 ;
             db.js ne traduit que des `?` positionnels (voir en-tête du fichier).
             `Object.values(row)` restitue l'ordre des propriétés tel que défini
             ci-dessus, qui est EXACTEMENT l'ordre des colonnes de la clause VALUES
             ci-dessus — aucune valeur n'est réordonnée, seule la forme de l'appel
             change. */
          await insSite.run(...Object.values(row));
          let last = null;
          for(let m=0;m<12;m++){
            ...
            await insMonth.run(id, YEAR, m, active, planned, done, planned ? partner : null,
                         done ? responsible : null);
            if(done){
              const date = `${YEAR}-${String(m+1).padStart(2,"0")}-${String(3+Math.floor(Math.random()*24)).padStart(2,"0")}`;
              await insVisit.run(newId("visit"), id, officeId[office], date, tag, responsible, "340943",
                Math.random() < 0.8 ? "Validé" : "À valider");
              last = date;
            }
          }
          if(!last && n % 6 === 0) last = `${YEAR-1}-${String(1+Math.floor(Math.random()*12)).padStart(2,"0")}-12`;
          if(last) await db.prepare("UPDATE sites SET last_visit=? WHERE id=?").run(last, id);
        }
      }
    }
    ...
    const insOut = db.prepare(`INSERT INTO outputs (id,activity_tag,year,month,planned,actual,adjust)
                               VALUES (?,?,?,?,?,?,?)`);
    for(const tag of ["URT","ACL","CAR","SMP","NTA","MPA"]){
      for(let m=0;m<=new Date().getMonth();m++){
        const planned = 4000 + Math.floor(Math.random()*9000);
        await insOut.run(newId("out"), tag, YEAR, m, planned,
    ...
    const insOc = db.prepare(`INSERT INTO outcomes (id,indicator_id,adm1,round_label,planned,value,collected_at,sample)
                              VALUES (?,?,?,?,?,?,?,?)`);
    for(const i of INDICATORS){
      for(const reg of ["Androy","Anosy","Atsimo-Andrefana","Vatovavy"]){
        const base = i[5]==="up" ? i[4]*(0.6+Math.random()*0.5) : i[4]*(0.85+Math.random()*0.5);
        await insOc.run(newId("oc"), indId[i[0]], reg, "Référence", i[4], r2(base), `${YEAR}-02-15`, 180+Math.floor(Math.random()*300));
        await insOc.run(newId("oc"), indId[i[0]], reg, "Suivi S1", i[4],
    ...
    for(const k of [...new Set(GEO.map(g=>g[2]))])
      await insPop.run(newId("pop"), k, "adm2", 40000+Math.floor(Math.random()*160000), 2.7);
    ...
    for(const mi of [Math.max(0,new Date().getMonth()-1), new Date().getMonth()]){
      for(const g of GEO){
        const [oname, code] = bureauDe[g[3]];
        for(const actType of ["GD","PREVMA"]){
          ...
          const pddRow = { id:newId("pdd"), year:YEAR, month:mi, wbs: actType==="GD"?"URT1":"URT2",
          ...
          /* Même remarque que pour insSite : paramètres nommés convertis en
             positionnels, ordre des propriétés inchangé = ordre des colonnes. */
          await insPdd.run(...Object.values(pddRow));
        }
      }
    }

    await db.prepare("INSERT INTO report_templates (id,name,blocks,intro) VALUES (?,?,?,?)")
    ...
    await db.prepare("INSERT INTO dashboards (id,name,widgets) VALUES (?,?,?)")
    ...
    await db.prepare("INSERT INTO odk_forms (id,name,form_id,project,kind,activity_tag,site_field,date_field) VALUES (?,?,?,?,?,?,?,?)")
    ...
    for(const [k,v] of Object.entries({ org:"Bureau pays", unit:"Unité suivi et évaluation", logo:"", currency:"MGA",
      dateFmt:"DD/MM/YYYY", pageSize:25, syncInterval:30, notifications:true,
      odkBase:"https://odk-central.example.org", opSize:"Large" }))
      await setS.run(k, JSON.stringify(v));
    ...
    {
      const gv0 = await db.prepare("SELECT id FROM geo_version WHERE is_current=1").get();
      const insScope = db.prepare(
        "INSERT INTO office_scope (office_id,geo_pcode) VALUES (?,?) ON CONFLICT (office_id,geo_pcode) DO NOTHING");
      for(const [office, idx] of Object.entries(ZONES)){
        if(!officeId[office]) continue;
        for(const i of idx){
          /* TODO-PG: resolveUnit() (lib/geo.js) — même réserve que writeVersion()
             ci-dessus : elle parle à la base via le pool, pas via le client de
             cette transaction. */
          const m = await resolveUnit({ adm1:GEO[i][1], adm2:GEO[i][2] }, gv0.id);
          if(m.pcode) await insScope.run(officeId[office], m.pcode);
        }
      }
    ...
    {
      const gv = await db.prepare("SELECT id FROM geo_version WHERE is_current=1").get();
      const lier = async (table, cols) => {
        const upd = db.prepare(`UPDATE ${table} SET geo_pcode=? WHERE id=?`);
        let n = 0;
        for(const row of await db.prepare(`SELECT * FROM ${table}`).all()){
          const names = {}; for(const [lvl, col] of Object.entries(cols)) names[lvl] = row[col];
          /* TODO-PG: resolveUnit() — même réserve que writeVersion() ci-dessus. */
          const m = await resolveUnit(names, gv.id);
          if(m.pcode){ await upd.run(m.pcode, row.id); n++; }
        }
        return n;
      };
      await lier("sites", { adm1:"adm1", adm2:"adm2", adm3:"adm3", adm4:"adm4" });
      await lier("pdd",   { adm1:"region", adm2:"district", adm3:"commune" });
    }
    ...
    const plannedBy = Object.fromEntries((await db.prepare(
    ...
    for(const c of await db.prepare(`SELECT pcode, name FROM geo_unit
      WHERE version_id=(SELECT id FROM geo_version WHERE is_current=1) AND level='adm3'`).all()){
      ...
      await insCl.run(newId("cl"), c.pcode, "adm3", YEAR, "", pop, hh, total, Math.round(total/4.8), SOURCE);
      /* Les activités se recouvrent : leur somme dépasse le total dédoublonné. */
      const brut = total / 0.72;
      for(const [tag,share] of [["URT",0.46],["NTA",0.26],["SMP",0.28]]){
        const t = Math.round(brut*share);
        await insCl.run(newId("cl"), c.pcode, "adm3", YEAR, tag, pop, hh, t, Math.round(t/4.8), SOURCE);
      }
    }
    ...
    for(const [ref,title,kind,purpose,method,_o,tag,resp,m0,m1,sample,note,lignes] of PLAN_MRE){
      const id = newId("mre");
      await insMre.run(id, YEAR, ref, title, kind, purpose, method, hqId, tag, resp, m0, m1, sample,
        m1 < moisCourant ? "realise" : m0 <= moisCourant ? "en_cours" : "planifie",
        "Ressources programme", note);
      for(const [cat,label,unit,qty,cost] of lignes){
        ...
        await insCost.run(newId("mrc"), id, cat, label, unit, qty, cost,
    ...
    for(const [i, office] of Object.keys(ZONES).entries()){
      const id = newId("mre");
      await insMre.run(id, YEAR, `MRE-B${String(i+1).padStart(2,"0")}`,
      ...
      for(const [cat,label,unit,qty,cost] of [["deplacement","Indemnités de déplacement","personne-jour",96,18],
       ["transport","Carburant et entretien","mois",12,340]])
        await insCost.run(newId("mrc"), id, cat, label, unit, qty, cost,
    ...
    {
      const gvG = await db.prepare("SELECT id FROM geo_version WHERE is_current=1").get();
      /* Les unités feuilles portent déjà des coordonnées (posées à la génération) :
         on remonte les cadres depuis elles. */
      const unites = await db.prepare(
      ...
      for(const u of unites){
        ...
        await insG.run(u.pcode, gvG.id, u.level, g, g, r5(b.w), r5(b.s), r5(b.e), r5(b.n));
        nG++;
      }
      await db.prepare(`UPDATE geo_version SET geom_units=?,
                  ...
                  geom_at=now() WHERE id=?`).run(nG, gvG.id);
    }
    ...
    {
      const gvT = await db.prepare("SELECT id FROM geo_version WHERE is_current=1").get();
      ...
      const contratDe = {};
      for(const [i, [nom, code, bureau, contact, plafond]] of PRESTATAIRES.entries()){
        const tid = newId("tpm");
        await insTpm.run(tid, nom, code, officeId[bureau] || null, contact,
          `${code.toLowerCase()}@prestataire.mg`);
        const cid = newId("tct");
        await insCtr.run(cid, tid, `TPM-${YEAR}-${String(i+1).padStart(2,"0")}`, plafond,
          `${YEAR}-01-01`, `${YEAR}-12-31`,
          "Suivi de processus et de distribution des activités du bureau");
        for(const [k, [driver, label, unit, cost]] of BAREME.entries())
          await insRate.run(newId("trt"), cid, driver, label, unit, cost, k);
        ...
        if(i === 0) await db.prepare(`INSERT INTO tpm_amendment (id,contract_id,ref,delta,reason,signed_at)
                                VALUES (?,?,?,?,?,?)`)
      ...
      for(const [pi, [nom, c]] of Object.entries(contratDe).entries()){
        /* Les zones du prestataire : les communes des districts de son bureau. */
        const idx = ZONES[c.bureau] || [];
        const communes = [];
        for(const i of idx){
          /* TODO-PG: resolveUnit() — même réserve que writeVersion() ci-dessus. */
          const m = await resolveUnit({ adm1:GEO[i][1], adm2:GEO[i][2], adm3:GEO[i][3] }, gvT.id);
          if(m.pcode) communes.push(m);
        }
        if(!communes.length) continue;

        for(const [m, statut, niveaux] of ETAPES){
          const pid = newId("tpp");
          await insPlan.run(pid, c.tid, c.cid, YEAR, m,
          ...
          for(const [zi, cm] of communes.entries()){
            const jours = 2 + (zi % 3) * 2;
            await insZone.run(newId("tpz"), pid, cm.pcode, ["URT","NTA","SMP"][zi % 3],
          ...
          /* TODO-PG: regenerate() (lib/tpm.js) — même réserve que writeVersion()
             ci-dessus : elle parle à la base via le pool, pas via le client de
             cette transaction. */
          await regenerate(pid);
          /* Les frais communs du mois, hors équipe — comme le groupe électrogène
             du classeur, qui ne dérive d'aucune quantité d'équipe. */
          await db.prepare(`INSERT INTO tpm_line (id,plan_id,zone_id,driver,label,unit,qty1,qty2,unit_cost,derived,sort)
                      VALUES (?,?,NULL,'forfait',?,?,?,1,?,0,900)`)
            .run(newId("tpl"), pid, "Groupe électrogène avec carburant", "groupe", 1, 80000);

          /* TODO-PG: planAmount() — même réserve que writeVersion() ci-dessus. */
          const montant = await planAmount(pid);
          for(const niv of niveaux)
            await insRev.run(newId("trv"), pid, niv, "valide",
          ...
          if(["valide_pays","cloture"].includes(statut)){
            const lignes = await db.prepare("SELECT id,qty1,qty2,unit_cost FROM tpm_line WHERE plan_id=?").all(pid);
            for(const [li, l] of lignes.entries()){
              if(li % 3 === 2) continue;   /* toutes les lignes ne sont pas encore réglées */
              const prevu = l.qty1 * l.qty2 * l.unit_cost;
              await insExp.run(newId("tex"), pid, l.id,
