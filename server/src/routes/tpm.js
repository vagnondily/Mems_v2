...
const audit = (req, entity, id, action, text) =>
...
r.get("/", (req, res) => {
  ...
  res.json({ rows: rows.map(t => {
    const contrats = db.prepare("SELECT * FROM tpm_contract WHERE tpm_id=? ORDER BY ref").all(t.id)
      .map(c => ({ id:c.id, ref:c.ref, currency:c.currency, status:c.status,
        start_date:c.start_date, end_date:c.end_date, note:c.note || "", rev:c.rev,
        solde: contractBalance(c.id),
        rates: db.prepare("SELECT * FROM tpm_rate WHERE contract_id=? ORDER BY sort, label")
          .all(c.id).map(x => ({ id:x.id, driver:x.driver, label:x.label, unit:x.unit || "",
            unit_cost:x.unit_cost, active:!!x.active, sort:x.sort })),
        amendments: db.prepare("SELECT * FROM tpm_amendment WHERE contract_id=? ORDER BY signed_at, rowid")
...
r.put("/:id", requireCap("admin"), (req, res) => {
  ...
  if(db.prepare("SELECT 1 FROM tpm WHERE name=? COLLATE NOCASE AND id<>?").get(b.name, cur.id))
    return res.status(409).json({ error:"un prestataire porte déjà ce nom" });
  db.prepare(`UPDATE tpm SET name=?, code=?, office_id=?, contact=?, email=?, phone=?,
              note=?, active=?, rev=rev+1, updated_at=datetime('now') WHERE id=?`)
    .run(b.name, b.code, b.office_id, b.contact, b.email, b.phone, b.note, b.active?1:0, cur.id);
  audit(req, "tpm", cur.id, "update", `Prestataire modifié — ${b.name}`);
  res.json({ ok:true });
});
...
r.delete("/:id", requireCap("admin"), (req, res) => {
  ...
  if(plans || comptes) return res.status(409).json({
    ...
    usage:{ plans, comptes } });
  db.prepare("DELETE FROM tpm WHERE id=?").run(cur.id);
  audit(req, "tpm", cur.id, "delete", `Prestataire supprimé — ${cur.name}`);
  res.json({ ok:true });
});
...
r.post("/contracts", requireCap("admin"), (req, res) => {
...
r.put("/contracts/:id", requireCap("admin"), (req, res) => {
  const cur = db.prepare("SELECT * FROM tpm_contract WHERE id=?").get(req.params.id);
  ...
  if(db.prepare("SELECT 1 FROM tpm_contract WHERE tpm_id=? AND ref=? AND id<>?")
...
r.post("/contracts/:id/amendments", requireCap("admin"), (req, res) => {
  const c = db.prepare("SELECT * FROM tpm_contract WHERE id=?").get(req.params.id);
  ...
  if(!p.success) return res.status(422).json({ error:"avenant invalide",
    details:p.error.issues.map(i => ({ champ:i.path.join("."), message:i.message })) });
  const b = p.data;
  const solde = contractBalance(c.id);
  ...
  if(b.delta < 0 && solde.plafond + b.delta < solde.engage)
    return res.status(409).json({
      ...
      solde });
  const id = newId("tam");
  db.prepare(`INSERT INTO tpm_amendment (id,contract_id,ref,delta,reason,signed_at,created_by)
              VALUES (?,?,?,?,?,?,?)`)
    .run(id, c.id, b.ref, b.delta, b.reason, b.signed_at, req.user.id);
  audit(req, "tpm_contract", c.id, "update",
    `Avenant ${b.ref || ""} au contrat ${c.ref} : ${b.delta > 0 ? "+" : ""}${b.delta} ${c.currency} — ${b.reason}`);
  res.status(201).json({ ok:true, id, solde: contractBalance(c.id) });
});
...
r.put("/contracts/:id/rates", requireCap("admin"), (req, res) => {
  const c = db.prepare("SELECT * FROM tpm_contract WHERE id=?").get(req.params.id);
  if(!c) return res.status(404).json({ error:"contrat introuvable" });
  ...
  tx(() => {
    db.prepare("DELETE FROM tpm_rate WHERE contract_id=?").run(c.id);
    const ins = db.prepare(`INSERT INTO tpm_rate (id,contract_id,driver,label,unit,unit_cost,active,sort)
                            VALUES (?,?,?,?,?,?,?,?)`);
    p.data.rates.forEach((x, i) => ins.run(newId("trt"), c.id, x.driver, x.label, x.unit,
  ...
  for(const pl of touches) regenerate(pl.id);
  audit(req, "tpm_contract", c.id, "update",
...
function visiblePlans(req, f){
  ...
  return db.prepare(`SELECT p.id FROM tpm_plan p JOIN tpm t ON t.id = p.tpm_id
...
r.get("/plans/:id", (req, res) => {
  const g = reachable(req, req.params.id);
  if(g.error) return res.status(g.status).json({ error:g.error });
  const p = planDetail(g.plan.id);
  p.actions = actionsFor(req, p);
  p.solde = contractBalance(g.plan.contract_id);
  res.json({ plan:p });
});
...
function actionsFor(req, p){
  const niveau = niveauAttendu(p.status);
  return {
    modifier: modifiable(p.status) && can(req.user, "edit") && peutEditer(req, p),
    soumettre: modifiable(p.status) && can(req.user, "edit") && peutEditer(req, p) && p.budget > 0,
    valider: !!niveau && peutValider(req, p, niveau),
    niveau,
...
function peutValider(req, p, niveau){
  const mien = tpmBound(req.user);
  if(niveau === "tpm"){
  ...
  return can(req.user, "admin") || (req.user.office_id && isNational(req.user.office_id));
}

r.post("/plans", requireCap("edit"), (req, res) => {
  const p = z.object({
  ...
  const c = db.prepare("SELECT * FROM tpm_contract WHERE id=? AND tpm_id=?")
...
r.put("/plans/:id/zones", requireCap("edit"), (req, res) => {
  const g = reachable(req, req.params.id);
  if(g.error) return res.status(g.status).json({ error:g.error });
  const plan = g.plan;
  if(!modifiable(plan.status)) return res.status(409).json({
    error:`ce plan est « ${plan.status} » : il n'est plus modifiable` });
  if(!peutEditer(req, { tpm_id:plan.tpm_id, office_id:plan.tpm_office }))
  ...
  tx(() => {
    ...
    db.prepare("DELETE FROM tpm_zone WHERE plan_id=?").run(plan.id);
    ...
    for(const z of p.data.zones)
      ins.run(newId("tpz"), plan.id, z.geo_pcode, z.activity_tag, z.team_label,
        z.supervisors, z.agents, z.days, z.travel_days, z.vehicles, z.fuel_litres,
        z.sites, z.note);
    regenerate(plan.id);
    db.prepare("UPDATE tpm_plan SET rev=rev+1, updated_at=datetime('now') WHERE id=?").run(plan.id);
  })();

  const d = planDetail(plan.id);
  audit(req, "tpm_plan", plan.id, "update",
...
r.put("/plans/:id/lines", requireCap("edit"), (req, res) => {
  const g = reachable(req, req.params.id);
  if(g.error) return res.status(g.status).json({ error:g.error });
  const plan = g.plan;
  if(!modifiable(plan.status)) return res.status(409).json({
  ...
  const zones = new Set(db.prepare("SELECT id FROM tpm_zone WHERE plan_id=?")
  ...
  tx(() => {
    db.prepare("DELETE FROM tpm_line WHERE plan_id=?").run(plan.id);
    const ins = db.prepare(`INSERT INTO tpm_line
      (id,plan_id,zone_id,driver,label,unit,qty1,qty2,unit_cost,derived,sort)
      VALUES (?,?,?,?,?,?,?,?,?,?,?)`);
    p.data.lines.forEach((l, i) => ins.run(newId("tpl"), plan.id, l.zone_id, l.driver, l.label,
...
r.put("/plans/:id", requireCap("edit"), (req, res) => {
  const g = reachable(req, req.params.id);
  if(g.error) return res.status(g.status).json({ error:g.error });
  const p = z.object({ ref:S(60), note:S(600) }).safeParse(req.body);
  if(!p.success) return res.status(422).json({ error:"plan invalide" });
  db.prepare("UPDATE tpm_plan SET ref=?, note=?, updated_at=datetime('now') WHERE id=?")
    .run(p.data.ref, p.data.note, g.plan.id);
  res.json({ plan: planDetail(g.plan.id) });
});

r.delete("/plans/:id", requireCap("del"), (req, res) => {
  const g = reachable(req, req.params.id);
  if(g.error) return res.status(g.status).json({ error:g.error });
  ...
  db.prepare("DELETE FROM tpm_plan WHERE id=?").run(g.plan.id);
  audit(req, "tpm_plan", g.plan.id, "delete", `Plan de suivi tiers supprimé`);
  res.json({ ok:true });
});

/* ── Le circuit ──────────────────────────────────────────────────────*/
r.post("/plans/:id/submit", requireCap("edit"), (req, res) => {
  const g = reachable(req, req.params.id);
  if(g.error) return res.status(g.status).json({ error:g.error });
  const plan = g.plan;
  if(!modifiable(plan.status)) return res.status(409).json({
  ...
  const solde = contractBalance(plan.contract_id);
  db.prepare("UPDATE tpm_plan SET status='soumis', rev=rev+1, updated_at=datetime('now') WHERE id=?")
    .run(plan.id);
  audit(req, "tpm_plan", plan.id, "update",
    `Plan soumis à validation — ${d.budget} ${d.currency}`);
  res.json({ plan: planDetail(plan.id),
...
r.post("/plans/:id/review", (req, res) => {
  const g = reachable(req, req.params.id);
  if(g.error) return res.status(g.status).json({ error:g.error });
  const plan = g.plan;
  const p = z.object({
  ...
  const d = planDetail(plan.id);
  if(!peutValider(req, d, niveau)) return res.status(403).json({
  ...
  tx(() => {
    db.prepare(`INSERT INTO tpm_review (id,plan_id,level,decision,comment,amount,user_id,user_label)
                VALUES (?,?,?,?,?,?,?,?)`)
      .run(newId("trv"), plan.id, niveau, p.data.decision, p.data.comment, d.budget,
           req.user.id, label(req.user));
    db.prepare("UPDATE tpm_plan SET status=?, rev=rev+1, updated_at=datetime('now') WHERE id=?")
      .run(suite, plan.id);
  })();
  audit(req, "tpm_plan", plan.id, "update",
  ...
  res.json({ plan: planDetail(plan.id), solde: contractBalance(plan.contract_id) });
});

r.post("/plans/:id/close", requireCap("validate"), (req, res) => {
  const g = reachable(req, req.params.id);
  if(g.error) return res.status(g.status).json({ error:g.error });
  if(g.plan.status !== "valide_pays") return res.status(409).json({
    error:"seul un plan validé au niveau pays peut être clôturé" });
  db.prepare("UPDATE tpm_plan SET status='cloture', rev=rev+1, updated_at=datetime('now') WHERE id=?")
    .run(g.plan.id);
  const d = planDetail(g.plan.id);
  audit(req, "tpm_plan", g.plan.id, "update",
    `Plan clôturé — budget ${d.budget}, dépensé ${d.spent ?? 0} ${d.currency}`);
  res.json({ plan:d });
});

/* ── Dépenses ────────────────────────────────────────────────────────*/
r.post("/plans/:id/expenses", requireCap("edit"), (req, res) => {
  const g = reachable(req, req.params.id);
  if(g.error) return res.status(g.status).json({ error:g.error });
  if(!["valide_pays","cloture"].includes(g.plan.status)) return res.status(409).json({
  ...
  if(!p.success) return res.status(422).json({ error:"dépense invalide",
    details:p.error.issues.map(i => ({ champ:i.path.join("."), message:i.message })) });
  const b = p.data;
  if(b.line_id && !db.prepare("SELECT 1 FROM tpm_line WHERE id=? AND plan_id=?")
      .get(b.line_id, g.plan.id))
    return res.status(422).json({ error:"cette ligne n'appartient pas au plan" });

  const id = newId("tex");
  db.prepare(`INSERT INTO tpm_expense (id,plan_id,line_id,spent_on,amount,ref,note,created_by)
              VALUES (?,?,?,?,?,?,?,?)`)
...
r.delete("/expenses/:id", requireCap("del"), (req, res) => {
  const e = db.prepare("SELECT * FROM tpm_expense WHERE id=?").get(req.params.id);
  if(!e) return res.status(404).json({ error:"dépense introuvable" });
  const g = reachable(req, e.plan_id);
  if(g.error) return res.status(g.status).json({ error:g.error });
  db.prepare("DELETE FROM tpm_expense WHERE id=?").run(e.id);
  audit(req, "tpm_plan", e.plan_id, "update", `Dépense annulée — ${e.amount}`);
  res.json({ plan: planDetail(e.plan_id) });
});
...
r.get("/suggest", (req, res) => {
  ...
  if(!q.success) return res.status(422).json({ error:"filtres invalides" });
  const bureau = officeBound(req.user) || q.data.office_id || null;
  res.json({ year:q.data.year, month:q.data.month,
    rows: suggestZones({ year:q.data.year, month:q.data.month, officeId:bureau }) });
});
