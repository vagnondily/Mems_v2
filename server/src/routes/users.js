...
const shape = (u) => ({ id:u.id, email:u.email, first_name:u.first_name, last_name:u.last_name,
  title:u.title, office_id:u.office_id, role:u.role, tabs:JSON.parse(u.tabs||"[]"),
...
r.post("/", validate(schemas.user), async (req, res) => {
  ...
  db.prepare(`INSERT INTO users (id,email,pw_hash,first_name,last_name,title,office_id,role,tabs,active,must_change_pw)
    ...
    .run(id, b.email, await hashPassword(b.password), b.first_name, b.last_name, b.title,
         b.office_id, b.role, JSON.stringify(b.tabs), b.active?1:0);
...
r.put("/:id", validate(schemas.user), async (req, res) => {
  ...
  db.prepare(`UPDATE users SET email=?, first_name=?, last_name=?, title=?, office_id=?,
              role=?, tabs=?, active=?, updated_at=datetime('now') ${pwSql} WHERE id=?`)
    .run(b.email, b.first_name, b.last_name, b.title, b.office_id, b.role,
