            ...
            <TableWrap>
              ...
              <tbody>{recap.map((g,i)=>(
                <tr key={g.id} className="hover:bg-sky-50">
                  ...
                  {/* Les exigences MRE sont de la PLANIFICATION du suivi, qu'un
                      éditeur peut être attribué à conduire (S6). Elles se persistent
                      désormais par la route dédiée `PUT /api/planning-config` (droit
                      éditeur), non par les réglages admin : l'édition est donc ouverte
                      à `edit`, et l'enregistrement suit le même droit. */}
  ...
  return (
    <>
      ...
      <Card flush title="Plan de collecte des indicateurs de résultat">
        <MonthLegend />
        {/* Le calendrier de collecte est de la PLANIFICATION : un éditeur peut le
            tenir (S6). Il se persiste dans sa table `outcome_plan` par la route
            dédiée en droit éditeur — l'édition suit donc `edit`. */}
