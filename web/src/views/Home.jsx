...
import { pddAgg, rate } from "./Planning.jsx";
import { PageHead } from "./Shell.jsx";

/* ══════════════════ Accueil ══════════════════ */
/* Forme héritée des tâches, conservée pour le générateur de rapports qui la
   consomme telle quelle (views/Reports.jsx). La dérivation, elle, a rejoint
   lib/taches.js : l'accueil n'en est plus propriétaire depuis que la cloche de
   la barre du haut s'en sert aussi. Sans compte, la vue est nationale — un
   rapport décrit la situation du pays, pas celle d'un poste de travail. */
const PRIO = { urgent:"haute", a_surveiller:"moyenne", information:"basse" };
const urgentTasks = (db) => tachesUtilisateur(db).map(t => ({
  id:t.id, prio:PRIO[t.severite], kind:t.titre, text:t.detail, ctx:t.contexte, go:t.vers }));
  ...
  return (
    <div className="space-y-4">
      <PageHead title="Accueil" text={`Bonjour ${me.firstName}. Situation consolidée du suivi au ${new Date().toLocaleDateString("fr-FR")}. `
        + "Les échéances à traiter sont signalées par la cloche, en haut de l'écran."}>
      ...
      {/* Le tableau des tâches urgentes tenait ici toute la largeur. Il n'était lisible
          que depuis l'accueil, alors qu'une échéance manquée doit se signaler d'où
          qu'on soit : il est devenu la cloche de la barre du haut. L'accueil garde ce
          pour quoi on l'ouvre — l'état d'ensemble, pas la liste des corvées. */}
